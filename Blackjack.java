 ...
 * Eli Giglietti
 * 
 * The purpose of this class is to represent a game of blackjack
 */
...
public class Blackjack {
    ...
    public void reset(){
        if (deck.size() < this.reshuffleCutoff){ //if the deck size is too small, then it remakes the deck
            this.deck = new Deck();
        }
        //deals two new hands
        this.playerHand = new Hand();
    ...
    public static void main(String[] args){
        Blackjack game = new Blackjack(17); //mekes a blakcjack object
        
        for(int i = 0; i<3; i++){ //runs three games of blackjack
            game.game(true);
            System.out.println("\n");
        }
    ...
    public int game(boolean verbose){
        reset();
        deal();
        if (verbose){ //if you put in true, it will print the games
            System.out.println(toString());
        }

        playerTurn(); //player goes
        if (playerHand.getTotalValue() > 21){ //check if player got over 21 

            return(-1);
        }
        
        dealerTurn(); //dealer goes
        if (dealerHand.getTotalValue() > 21){ //check if dealer goes over 21
        ...
        //finds out who wins based on higher value if neither went over 21
        ...
        if (playerHand.getTotalValue() < dealerHand.getTotalValue()){
            ...
            return (0); //tie if the values are the same
        }
